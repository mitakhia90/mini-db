...
## Новые команды и тесты (дополнение)

В этой ветке/версии добавлены небольшие улучшения в CLI и storage-слой:

- Новые команды в CLI (`DefaultEngine`):
  - `HELP` — вывод списка доступных команд;
  - `LIST TABLES` — вывод всех таблиц из каталога;
  - `DELETE FROM <table> WHERE <column> = <value>` — простая реализация удаления строк по равенству.

- Улучшения хранения/кэша (`DefaultOperationManager`):
  - Добавлен простой кэш последней страницы с достаточным свободным местом (чтобы ускорять последовательные вставки);
  - Корректная инициализация новых страниц на диске (пустая HeapPage с заголовком);
  - Метод `delete` реализован (сканирование страниц и компактизация при удалении).

- Новые/обновлённые unit-тесты:
  - `OperationManagerInsertBoundaryTest` — проверяет поведение при переполнении страницы и вставку слишком большой строки;
  - `OperationManagerCacheBehaviorTest` — проверяет, что повторные вставки используют кэш и не выполняют полный скан;
  - `InsertNoSpaceRegressionTest` и тесты для DROP/каталога — дополнительные проверки хранения/каталога.

Эти изменения направлены на повышение надёжности и читаемости поведения операций записи/чтения и удобства использования CLI.
